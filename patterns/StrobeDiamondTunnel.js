...
export class StrobeDiamondTunnel extends PatternBase {
    ...
    _initializeDiamonds() {
        ...
        // Center diamond
        this.diamonds.push(new DiamondElement(0, this.config.baseDiamondSize, this.currentDiamondColor));
    ...
    _updateAnimation(visualParams) {
        ...
        if (this.beatBurst > 0) {
            ...
            if (this.beatBurst < 0.01) this.beatBurst = 0;
        }
    }
    ...
    render(renderer, network, audioData) {
        ...
        const bgGradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, this.config.glowRadius);
    ...
    _renderRadialLines(ctx, canvas) {
        ...
        for (let i = 0; i < numDirections; i++) {
            ...
            for (let j = 0; j < chunksPerLine; j++) {
                ...
                if (startDist > maxRadius) continue;
